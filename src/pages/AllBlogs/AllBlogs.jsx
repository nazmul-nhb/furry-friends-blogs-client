...
const AllBlogs = () => {
	...
	const {
		...
		queryFn: async () => {
			const res = await axiosPublic.get(
				`/blogs?sort=${sortByTime ? -1 : 1}&page=${
					currentPage - 1
				}&size=${itemsPerPage}&category=${
					category === "All" ? " " : category
				}&search=${searchText}`
			);
			return res.data;
		...
		<section className="mx-2 md:mx-8 my-2 md:my-8 p-2 md:px-4">
			...
			<div className="my-6 text-sm md:text-base flex flex-col md:flex-row justify-start md:justify-center items-center gap-3 md:gap-6">
				...
				<form
					...
					className="flex gap-2 items-center text-furry"
				>
					<div className="flex gap-2 items-center relative">
						<input
							ref={inputRef}
							defaultValue={searchText}
							className="text-left p-2 rounded-lg outline outline-none border bg-transparent focus:border-2 text-furry border-furry"
						...
						{searchText !== "" && (
							<button
								...
								className="absolute right-2 text-3xl hover:text-red-900"
							>
								<FaDeleteLeft />
							</button>
						)}
					...
					<button
						className="border py-2 px-4 rounded-lg font-bold tracking-wider border-furry bg-furry text-white hover:bg-transparent hover:text-furry transition-all duration-700"
						type="submit"
					>
						Search
					</button>
			...
			{/* Total Blog Count */}
			{blogCount > 0 && !searchText && (
				<h3 className="text-center text-furry font-kreonSerif text-xl md:text-3xl mb-6 flex items-center justify-center">
					Total {blogCount} {blogCount > 1 ? "Blogs" : "Blog"} Posted{" "}
					{category &&
						category !== "All" &&
			...
			{/* Show Search Count */}
			{searchText && blogs?.length > 0 ? (
				<div className="mb-8 flex items-center justify-center text-furry font-kreonSerif max-[430px]:text-xl text-2xl">
					{blogs?.length} {blogs?.length > 1 ? "Matches" : "Match"}{" "}
					Found
					{category &&
						category !== "All" &&
						` in "${category}" Category`}
				...
				<div className="text-center flex flex-col items-center justify-center text-furry font-kreonSerif max-[430px]:text-xl text-2xl md:text-3xl">
			...
			{blogs?.length > 0 && (
				<>
					...
					{/* Sort by Time Posted */}
					{blogs?.length > 1 && (
						<div className="flex gap-2 items-center justify-center font-medium text-sm md:text-xl mb-4">
							<h3 className="text-furry">
								{sortByTime
									? "You Are Seeing Latest Posts First"
									: "You Are Seeing Oldest Posts First"}
							</h3>
							<button
								onClick={() => setSortByTime(!sortByTime)}
								className="border py-0.5 px-2 rounded-lg border-furry bg-transparent text-furry hover:bg-furry text-xs md:text-lg hover:text-white transition-all duration-700"
								type="submit"
							>
								{sortByTime
									? "See Oldest Posts"
					...
					<div className="flex flex-col gap-4 justify-center items-center font-semibold mt-8 lg:mt-16">
						<p className="text-furry">
							Page: {currentPage} of {totalPages}
						</p>
						<div className="flex gap-3">
							<button
								className={
									"px-3 border disabled:text-gray-500 disabled:border-gray-500 disabled:hover:text-gray-500 disabled:hover:bg-transparent text-furry border-furry hover:bg-furry hover:text-white"
								}
							...
							{pages?.map((page) => (
								<button
									className={`px-3 border ${
										currentPage === page + 1
											? "bg-furry border-furry text-white hover:bg-transparent hover:text-furry"
											: " text-furry border-furry hover:bg-furry hover:text-white"
									}`}
									onClick={() => setCurrentPage(page + 1)}
									key={page}
								>
							...
							<button
								className={
									"px-3 border disabled:text-gray-500 disabled:border-gray-500 disabled:hover:text-gray-500 disabled:hover:bg-transparent disabled:cursor-not-allowed text-furry border-furry hover:bg-furry hover:text-white"
								}
								disabled={
						...
						<select
							className="border px-2 py-1 focus:text-furry outline-furry border-furry text-furry bg-transparent focus:border-2"
							value={itemsPerPage}
