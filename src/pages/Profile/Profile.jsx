...
const Profile = () => {
	...
	const {
		...
		queryFn: async () => {
			const res = await axiosPublic.get(
				`/blogs?currentUser=${user?.email}&sort=-1`
			);
			return res.data;
		},
	});
	...
	const handleDeleteBlog = (id, blog_title) => {
		Swal.fire({
			...
			if (result.isConfirmed) {
				setDeleting(true);
				axiosSecure
					.delete(`/blog/${id}?email=${user?.email}`)
					.then((res) => {
	...
	return (
		<section className="mx-2 md:mx-8 my-2 md:my-8 p-2 md:px-4 space-y-6">
			<Helmet>
				<title>
					Profile : {user?.displayName} - Furry Friends Blogs
				</title>
			</Helmet>
			<div className="flex flex-col xl:flex-row justify-between gap-10 items-center mb-8 xl:mb-16">
				<div className="w-full lg:w-2/3 flex-1 border bg-gradient-to-l from-[#2e50bc62] to-[#033eff37]  border-furry flex flex-col gap-6 p-6 shadow-lg shadow-[#8689ee]">
					<div className="flex flex-col lg:flex-row gap-2 items-center md:justify-start justify-center my-4">
						...
						<img
							className="user-name user-email border p-1 border-furry w-24 md:w-36 h-24 md:h-36"
							src={user?.photoURL}
						...
						<div className="flex flex-col justify-center items-center md:items-start md:justify-start gap-3">
							...
							<Tooltip
								anchorSelect=".verification-status"
								place="top"
							>
								{user?.emailVerified
									? "Verified!"
							...
							<h4 className="verification-status font-semibold flex items-center gap-1">
								{user?.email}{" "}
								{user?.emailVerified ? (
									<MdVerified className="text-furry" />
								) : (
									<VscUnverified className="text-red-700" />
								)}
							</h4>
				...
				<div
					className={`w-full lg:w-1/3 bg-gradient-to-l from-[#2e50bc62] to-[#033eff37] border border-furry shadow-lg shadow-[#8689ee] p-4 justify-between ${
						usersBlogs?.length > 0
							? "flex flex-col items-center"
							: "hidden"
					} gap-2 text-center`}
				>
					<h3 className="text-furry font-kreonSerif text-xl md:text-2xl">
						You have Posted {usersBlogs?.length}{" "}
						{usersBlogs?.length > 1 ? "Blogs" : "Blog"} So Far!
					</h3>
					<Link to={`/add-blog`}>
						<Button
							buttonText={"Add A New Blog"}
							hoverBgColor={"transparent"}
							hoverColor={"white"}
							color={"#1e40ad"}
							className={
								"my-2 border px-4 py-1 font-bold text-xl"
							}
						></Button>
					</Link>
				</div>
			...
			{usersBlogs?.length <= 0 ? (
				...
				<div>
					<h3 className="text-furry font-kreonSerif text-2xl md:text-3xl mb-6">
						Your Blogs:
					</h3>
