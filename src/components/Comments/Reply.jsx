...
const Reply = ({
	...
	return (
		<div className="ml-8">
			...
			<div className="flex items-center gap-1">
				...
				<div className="flex flex-col gap-0 leading-4">
					<h5 className="text-gray-500 text-xs">
						{moment(replied_on).format(
							"MMMM DD, YYYY [at] hh:mm:ss A"
						)}
					</h5>
					<h4 className="font-normal" title={reply_email}>
			...
			{editableReply ? (
				<form
					className="ml-11 mb-2 flex items-start flex-col gap-4"
					onSubmit={handleEditReply}
				>
					<textarea
						defaultValue={reply_body}
						className="w-full lg:w-2/5 h-28 border border-furry rounded-lg p-2 outline-none focus:border-2"
						name="edit_reply"
						id="edit_reply"
						placeholder={`Edit Your Reply to ${commenter_name}'s Comment`}
					></textarea>
					...
					<div className="flex gap-6 items-center">
						...
						></Button>
						<Button
							onClick={() => setEditableReply(!editableReply)}
							buttonText={"Cancel"}
							color={"gray"}
							hoverBgColor={"transparent"}
							hoverColor={"white"}
			...
			<div className="text-xs ml-11 flex items-center gap-2 mb-2 font-semibold text-blue-500">
				...
				{updated_on && (
					<button
						className="reply-edit-time cursor-pointer hover:text-furry"
						onClick={() =>
							setShowReplyUpdateTime(!showReplyUpdateTime)
						}
					>
						Edited
					</button>
			...
			{showReplyUpdateTime && updated_on && (
				<h5 className="ml-11 text-gray-500 text-sm flex items-center gap-1">
					<MdOutlineUpdate />
