...
const Comment = ({
	...
	const commentTime = moment(commented_on).format(
				...
				axiosSecure
					...
					.then((res) => {
						if (res.data.deletedCount > 0) {
							refetch();
							Swal.fire("Removed!", "Reply Deleted!", "success");
	...
	const handleEditComment = (e) => {
		...
		axiosSecure
			.patch(`/comment/${_id}`, { ...editedComment })
			.then((res) => {
				// console.log(res.data);
		...
		<div className="my-4">
			...
			<div className="flex items-center gap-1">
				<img
					src={commenter_photo}
					alt={commenter_name}
					className="w-11 h-11 rounded-full p-[2px] border"
				/>
				<div className="flex flex-col justify-around gap-0 items-start leading-none">
					...
					<h4
						...
						title={commenter_email}
					>
						<span className="font-bold text-furry">
							{commenter_name}
						</span>{" "}
						commented:
					</h4>
			...
			{editableComment ? (
				<form
					className="ml-12 mb-2 flex flex-col items-start gap-4"
					onSubmit={handleEditComment}
				>
					<textarea
						defaultValue={comment_body}
						className="w-full lg:w-3/5 h-32 border border-furry rounded-lg p-2 outline-none focus:border-2"
						name="edit_comment"
						id="edit_comment"
			...
			<div className="text-xs ml-12 flex items-center gap-2 mb-2 font-semibold text-blue-500">
				{/* Edit & Delete Buttons */}
				{user?.email === commenter_email && (
					<div className="flex gap-2">
						<button
							className="cursor-pointer hover:text-furry"
							onClick={() => setEditableComment(!editableComment)}
						>
							Edit
						</button>
						<button
				...
				{updated_on && (
					<button
						className="comment-edit-time cursor-pointer hover:text-furry"
						onClick={() =>
							setShowCommentUpdateTime(!showCommentUpdateTime)
						}
					>
						Edited
					</button>
			...
			{showCommentUpdateTime && updated_on && (
				<h5 className="ml-12 mb-2 text-gray-500 text-sm flex items-center gap-1">
					<MdOutlineUpdate />{" "}
					{moment(updated_on).format("MMMM DD, YYYY [at] hh:mm:ss A")}
				</h5>
			)}
			{/* Reply Box */}
			<div className="ml-4 mb-1">
				{showReplyBox && (
					<form
						className="ml-6 mb-2 flex items-start flex-col gap-4"
						onSubmit={handlePostReply}
					>
						<textarea
							ref={replyRef}
							className="w-full lg:w-2/5 h-28 bg-transparent border border-furry rounded-lg p-2 outline-none focus:border-2"
							name="reply"
							id="reply"
							placeholder={`Reply to ${commenter_name}'s Comment`}
						></textarea>
						...
						<div className="flex gap-6 items-center">
							<Button
								...
								hoverColor={"white"}
								className={
									"text-sm border rounded-xl px-3 py-1 font-medium"
								}
							></Button>
							<Button
