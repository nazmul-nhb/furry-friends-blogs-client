...
const Blog = ({
		...
		if (user && user?.email) {
			axiosSecure
				.post("/wishlist", {
					...
					time_added: moment().format("YYYY-MM-DD HH:mm:ss"),
				})
				.then((res) => {
	...
	return (
		<div className="border p-3 flex flex-col shadow-md shadow-[#8689ee]">
			<Link to={`/blog-details/${_id}`}>
				<figure className="relative border p-1 mb-2 hover:scale-[1.03] transition-all duration-700">
					<img
						...
						alt={blog_title}
					/>
					<h3 className="absolute bottom-1 left-1 pb-1 pt-[2px] pl-2 w-full bg-gradient-to-r from-furry via-[#1e3fada3] to-[#1e3fad01] font-jokeyOneSans tracking-wider text-lg flex items-center gap-1 text-white">
						<MdPets /> Category:
						{
			...
			{/* <hr className="my-2" /> */}
			<Link to={`/blog-details/${_id}`}>
				<h3 className="font-kreonSerif hover:text-furry transition-all duration-700 text-xl md:text-2xl">
					{blog_title}
				</h3>
			</Link>
			<p className={theme === "dark" ? "text-gray-400" : "text-gray-500"}>
				Posted by{" "}
				<span className="text-furry font-semibold">
			...
			<div className=" rounded-b-xl flex justify-between items-center">
				{profile ? (
					<Button
						onClick={() => handleDeleteBlog(_id, blog_title)}
						className={
							"border px-3 md:px-4 lg:px-3 xl:px-6 py-2 rounded-3xl text-xs md:text-sm lg:text-xs xl:text-base font-bold flex items-center gap-1"
						}
						color={"#1e40ad"}
				...
				{wishlist ? (
					<Button
						onClick={() => handleDeleteWishlist(_id, blog_title)}
						className={
							"border px-3 md:px-4 lg:px-3 xl:px-6 py-2 rounded-3xl text-xs md:text-sm lg:text-xs xl:text-base font-bold flex items-center gap-1"
						}
						color={"#1e40ad"}
